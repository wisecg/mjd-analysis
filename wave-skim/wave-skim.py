...
def main(argv):
    ...
    """
    # runList = "./data/Debug.txt"
    runList = "./data/m1Cal.txt"

    outFile = "./data/wave-m1Cal.root"

    # gatCut = "channel%2==0 && trapENFCal < 4000 && trapENFCal > 1 && (trapENFCal > 800 || trapENFCal < 500) && !wfDCBits"

    # ben says you need to add an a/e cut
    gatCut = "trapENFCal > 1591 && trapENFCal < 1593 && !wfDCBits && channel%2==0"

    # GatGrabber(runList, gatCut, outFile, writeET,"",-1)  # "test", or ""
...
def SeymourSkimmer(theCut, skimLoc, outFile, writeET, useFile, entryLimit=-1):
    ...
    triFilt = np.zeros(1,dtype=float)
    eventOut = MGTEvent()
    ...
    waveTree.Branch("d2wf0MHzTo50MHzPower",pwr,"d2wf0MHzTo50MHzPower/D")  # new from micah
            ...
            for i in xrange(numWFs):
                try:
                    ...
                    if chan in evt[3]:
                        ...
                        theRun[0] = run
                        gatEvt[0] = long(evt[1])
                        pwr[0] = lib.d2wf0MHzTo50MHzPower.at(i)
                        triFilt[0] = lib.s_kvorrT.at(i)
                        waveTree.Fill()
                except:
...
def GatGrabber(runList, gatCut, outFile, writeET, action="test", maxwfs=-1):
    print "Starting wave-skim.  Cutting on: \n\n",gatCut,"\n"
